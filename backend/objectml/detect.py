...
def detect_direction(lane1, lane2):
    """
    Detect the driving direction based on lane lines.
    
    Args:
        lane1: Tuple containing lane 1 coordinates (x1, y1, x2, y2).
        lane2: Tuple containing lane 2 coordinates (x1, y1, x2, y2).

    Returns:
        Direction as a string ('left', 'right', 'forward', 'other').
    """
    # Calculate the midpoint of the two lanes
    midpoint = ((lane1[0] + lane1[2]) / 2 + (lane2[0] + lane2[2]) / 2) / 2

    # Define the region for forward movement
    forward_region = (350, 600)

    if midpoint < forward_region[0]:
        return "left"
    elif midpoint > forward_region[1]:
        return "right"
    else:
        return "forward"
...
def run(screen):
    ...
    try:
        ...
        if results is not None:
            direction = detect_direction((l1inx1, 600, l1inx2, 350), (l2inx1, 600, l2inx2, 350))
            cv2.putText(screen, f"Direction: {direction}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
